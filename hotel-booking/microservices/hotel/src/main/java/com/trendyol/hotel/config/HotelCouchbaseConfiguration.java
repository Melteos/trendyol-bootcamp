...
@Configuration
public class HotelCouchbaseConfiguration {

    private final CouchbaseProperties couchbaseProperties;

    public HotelCouchbaseConfiguration(CouchbaseProperties couchbaseProperties) {
    ...
    @Bean
    public Collection hotelCollection(Cluster couchbaseCluster) {
    ...
    @Bean
    public Collection roomBookingCollection(Cluster couchbaseCluster) {
        couchbaseCluster.bucket(couchbaseProperties.getBucketName()).waitUntilReady(Duration.ofSeconds(122));
        return couchbaseCluster.bucket(couchbaseProperties.getBucketName()).defaultCollection();
    }
}
